...
yaudit_expect           (char a_type, char b_owner [LEN_USER], char b_group [LEN_USER], char b_perms [LEN_TERSE], int *r_uid, int *r_gid, int *r_prm, char r_disp [LEN_LABEL])
{
   ...
   /*---(user ownership)-----------------*/
   ySCORE_mark (myAUDIT.m_yscore, "EOWNER"  , '-');
   --rce;  if (b_owner != NULL && strcmp (x_owner, "-") != 0) {
      ...
      if (rc >= 0) {
         x = atoi (t);
         if      (strcmp (t      , "@"    ) == 0)  ySCORE_mark (myAUDIT.m_yscore, "EOWNER"  , 'O');  /* default  */
         else if (strcmp (t      , "0"    ) == 0)  ySCORE_mark (myAUDIT.m_yscore, "EOWNER"  , 'ö');  /* specific */
         else if (x > 0)                           ySCORE_mark (myAUDIT.m_yscore, "EOWNER"  , 'ö');  /* specific */
         else                                      ySCORE_mark (myAUDIT.m_yscore, "EOWNER"  , 'o');  /* standard */
      } else {
   ...
   /*---(group ownership)----------------*/
   ySCORE_mark (myAUDIT.m_yscore, "EGROUP"  , '-');
   --rce;  if (b_group != NULL && strcmp (x_group, "-") != 0) {
      ...
      if (rc >= 0) {
         x = atoi (t);
         if      (strcmp (t      , "@"    ) == 0)  ySCORE_mark (myAUDIT.m_yscore, "EGROUP"  , 'G');  /* default  */
         else if (strcmp (t      , "0"    ) == 0)  ySCORE_mark (myAUDIT.m_yscore, "EGROUP"  , 'ê');  /* specific */
         else if (x > 0)                           ySCORE_mark (myAUDIT.m_yscore, "EGROUP"  , 'ê');  /* specific */
         else                                      ySCORE_mark (myAUDIT.m_yscore, "EGROUP"  , 'g');  /* standard */
      } else {
   ...
   /*---(permissions)--------------------*/
   ySCORE_mark (myAUDIT.m_yscore, "EPERMS"  , '-');
   --rce;  if (b_perms != NULL && strcmp (x_perms, "-") != 0) {
      ...
      if (rc >= 0) {
         if      (strcmp (t      , "@"    ) == 0)  ySCORE_mark (myAUDIT.m_yscore, "EPERMS"  , 'P');  /* default  */
         else if (strcmp (x_perms, "(n/a)") == 0)  ySCORE_mark (myAUDIT.m_yscore, "EPERMS"  , '÷');  /* specific */
         else                                      ySCORE_mark (myAUDIT.m_yscore, "EPERMS"  , 'p');  /* standard */
      } else {
   ...
   /*---(save-back)----------------------*/
   if (strchr ("Oöo", ySCORE_value (myAUDIT.m_yscore, "EOWNER"  )) != NULL) {
      if (r_uid   != NULL)  *r_uid = x_uid;
      if (b_owner != NULL)  snprintf (b_owner, LEN_USER , "%s", x_owner);
   }
   if (strchr ("Gêg", ySCORE_value (myAUDIT.m_yscore, "EGROUP"  )) != NULL) {
      if (r_gid   != NULL)  *r_gid = x_gid;
      if (b_group != NULL)  snprintf (b_group, LEN_USER , "%s", x_group);
   }
   if (strchr ("P÷p", ySCORE_value (myAUDIT.m_yscore, "EPERMS"  )) != NULL) {
...
yaudit_extra            (char a_type, int a_major, int a_minor, char a_ttype, char a_target [LEN_PATH], int a_epoch, long a_bytes, int a_inode, char a_hash [LEN_DESC])
{
   ...
   --rce;  if (strchr ("bc", a_type) != NULL) {
      if (a_major >= 0 && a_major <= 255)    ySCORE_mark (myAUDIT.m_yscore, "EMAJOR"  , 'j');
      else {
         ySCORE_mark (myAUDIT.m_yscore, "EMAJOR"  , '°');
   ...
   --rce;  if (strchr ("bc", a_type) != NULL) {
      if (a_minor >= 0 && a_minor <= 255)    ySCORE_mark (myAUDIT.m_yscore, "EMINOR"  , 'n');
      else {
         ySCORE_mark (myAUDIT.m_yscore, "EMINOR"  , '°');
   ...
   --rce;  if (strchr ("sh", a_type) != NULL) {
      ...
      if (strcmp (x_tdesc, "WTF") != 0)   ySCORE_mark (myAUDIT.m_yscore, "ETTYPE"  , a_ttype);
      else {
         ySCORE_mark (myAUDIT.m_yscore, "ETTYPE"  , '°');
   ...
   /*---(target name)--------------------*/
   --rce;  if (strchr ("sh", a_type) != NULL) {
      ySCORE_mark (myAUDIT.m_yscore, "ETARGET" , '°');
      if (a_target != NULL && strcmp (a_target, "") != 0) {
         ...
         if (a_target [0] != '/') {
            ...
            if (x_bad != 'y')   ySCORE_mark (myAUDIT.m_yscore, "ETARGET" , 't');
         }
      }
   ...
   /*---(specialty)----------------------*/
   if (a_epoch > 0)  { ySCORE_mark (myAUDIT.m_yscore, "EEPOCH"  , 'e');  x_hack = 'y';  }
   if (a_bytes > 0)  { ySCORE_mark (myAUDIT.m_yscore, "EBYTES"  , 'b');  x_hack = 'y';  }
   if (a_inode > 0)  { ySCORE_mark (myAUDIT.m_yscore, "EINODE"  , 'i');  x_hack = 'y';  }
   --rce;  if (a_hash != NULL && strcmp (a_hash, "") != 0) {
      ySCORE_mark (myAUDIT.m_yscore, "EHASH"   , '°');
      l = strlen (a_hash);
      if (l == 40) {
         ySCORE_mark (myAUDIT.m_yscore, "EHASH"   , '#');
         x_bad = '-';
         ...
         if (x_bad != 'y')  {  ySCORE_mark (myAUDIT.m_yscore, "EHASH"   , 'h'); x_hack = 'y'; }
